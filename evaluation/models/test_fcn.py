...
def process_results(output_predictions,tasks=[],latency=0, percentage=1,categories=[],convert=False,inference=False):
    ...
    # labels = {categories[id]:1 for id in ids}
    labels = {}
    for id in ids:
        n = categories[id]
        # if n not in tasks:
...
def inference(filename, model,tasks=[],latency=0, percentage=1,categories=[],inference=False):
    ...
    # time.sleep(latency)
    labels = process_results(output_predictions,tasks,latency, percentage,categories,inference=inference)
    return labels
...
if __name__ == "__main__":
    ...
    for i,param_row in query_parameters.iloc[91:].iterrows():
    ...
    # output_predictions = inference(model,filename)
